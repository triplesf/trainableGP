...
def eaSimple(randomseed, population, toolbox, cxpb, mutpb, elitpb, ngen , stats=None,
             halloffame=None, verbose=__debug__, logger=None):
    ...
    for i, t in record.items():
        for ri, rt in t.items():
            # record[i][ri] = "{}    ".format(round(rt, 2))
            record[i][ri] = "{:.2f}".format(rt)
            # record[i][ri] = "0.00"

    testResults = evalTest(halloffame[0])
    ...
    for gen in range(1, ngen + 1):
        ...
        # train_tf, test_tf, trainLabel, testL, testResults, trainResults = evalTest(toolbox, halloffame[0],
    ...
    test_result = evalTest(halloffame[0])
